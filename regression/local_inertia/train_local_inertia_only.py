...
if __name__ == "__main__":
    ...
    training_features = np.column_stack((eig0, eig1, eig2, log_mass))
    ...
    clf = ml.MLAlgorithm(training_features, method="regression", cross_validation=cv, split_data_method=None, n_jobs=60,
                         save=True, path=saving_path + "inertia_only/classifier/classifier.pkl", param_grid=param_grid)
    if cv is True:
        print(clf.best_estimator)
        print(clf.algorithm.best_params_)
        print(clf.algorithm.best_score_)

    np.save(saving_path + "inertia_only/f_imp.npy", clf.feature_importances)