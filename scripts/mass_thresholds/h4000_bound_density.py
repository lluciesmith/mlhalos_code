...
RF = ml.MLAlgorithm(features_training, split_data_method=None, n_jobs=60, save=True,
                    path=save_dir + "classifier/classifier.pkl")
clf = RF.algorithm
print(RF.best_estimator)
np.save(save_dir + "feature_importances.npy", RF.feature_importances)

pred_prob = RF.algorithm.predict_proba(den_features[testing_index])
np.save(save_dir + "predicted_probabilities.npy", pred_prob)
true_test_label = true_labels[testing_index]
np.save(save_dir + "true_test_labels.npy", true_test_label)
