...
save_dir = "/share/data1/lls/h4000_in_out_boundary/"
...
ic = parameters.InitialConditionsParameters(load_final=True, max_halo_number=4000)
...
ell_features = np.lib.format.open_memmap(
    "/share/data1/lls/shear_quantities/quantities_id_ordered/density_subtracted_ellipticity.npy",
                                         mode="r", shape=(num_ids, 50))
prol_features = np.lib.format.open_memmap(
    "/share/data1/lls/shear_quantities/quantities_id_ordered/density_subtracted_prolateness.npy",
                                         mode="r", shape=(num_ids, 50))

try:
    clf = joblib.load(save_dir + "shear_den_classifier/classifier.pkl")

except IOError:
    den_training = den_features[training_index]
    ell_training = ell_features[training_index]
    prol_training = prol_features[training_index]
    features_training = np.column_stack((den_training, ell_training, prol_training, true_labels[training_index]))

    RF = ml.MLAlgorithm(features_training, split_data_method=None, n_jobs=60, save=True,
                        path=save_dir + "shear_den_classifier/classifier.pkl")
    clf = RF.algorithm
    print(RF.best_estimator)
    np.save(save_dir + "shear_density/feature_importances.npy", RF.feature_importances)

features_testing = np.column_stack((den_features[testing_index], ell_features[testing_index],
                                    prol_features[testing_index]))
pred_prob = clf.predict_proba(features_testing)
np.save(save_dir + "shear_density/predicted_probabilities.npy", pred_prob)
true_test_label = true_labels[testing_index]
np.save(save_dir + "shear_density/true_test_labels.npy", true_test_label)