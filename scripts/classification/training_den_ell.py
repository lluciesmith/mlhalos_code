...
# train only on density+ellipticity

training_features = np.column_stack((training_den_shear_features[:, :100], training_den_shear_features[:, -1]))

trained_algo = ml.MLAlgorithm(training_features, split_data_method=None, n_jobs=60,
                              save=True, path="/home/lls/stored_files/shear_and_density/den+ell/classifier/classifier"
                                              ".pkl")
...
np.save("/home/lls/stored_files/shear_and_density/den+ell/feature_importances.npy", trained_algo.feature_importances)